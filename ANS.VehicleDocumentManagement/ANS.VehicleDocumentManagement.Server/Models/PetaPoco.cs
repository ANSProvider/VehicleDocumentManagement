...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
                    ...
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        if (columns == null)
                        {
                            foreach (var i in pd.Columns)
                            {
                                ...
                                foreach (String item in primaryKeyName.Split(','))
                                {
                                    if (string.Compare(i.Key, item, true) == 0)
                                    {
                                        if (pkindex > 0)
                                            mwhere.Append(" And ");
                                        pkindex++;
                                        mwhere.AppendFormat("{0} = {1}{2}", _dbType.EscapeSqlIdentifier(i.Key), _paramPrefix, index++);
                                        AddParam(cmd, i.Value.GetValue(poco), i.Value.PropertyInfo);
                                        iscontinue = true;
                                        continue;
                                    }
                        ...
                        cmd.CommandText = string.Format("UPDATE {0} SET {1} WHERE {2} ",
                                            _dbType.EscapeTableName(tableName), sb.ToString(), mwhere.ToString());
        ...
        public int Delete(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        var sql = string.Format("DELETE FROM {0} WHERE {1}", _dbType.EscapeTableName(tableName), mwhere.ToString());
                        cmd.CommandText = sql;
            ...
            // Do it
            // var sql = string.Format("DELETE FROM {0} WHERE {1}=@0", _dbType.EscapeTableName(tableName), mwhere.ToString());
        ...
        public bool IsNew(string primaryKeyName, object poco)
        {
            ...
            //change for the Update With multiple primary key
            //else if (primaryKeyName.Contains(","))
            //{
            //    return primaryKeyName.Split(',')
            //        .Select(pkPart => GetValue(pkPart, poco))
